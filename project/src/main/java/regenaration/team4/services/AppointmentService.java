...
public class AppointmentService {
    ...
    public List<Appointment> getFilteredCitizenAppointments(@RequestParam("fDate") Date fromDate, @RequestParam("tDate") Date toDate, @RequestParam("specialtyId") Integer specialtyId, Principal principal){
        ...
        for (int i = 0; i < appointments.size(); i++) {
            ...
            if ( ((appointments.get(i).getAppointment_date().compareTo(fromDate)) > 0) || ((appointments.get(i).getAppointment_date().compareTo(toDate)) < 0) || (mySpecialtyId != specialtyId) ) {
